...
const P1Hand = () => {
    ...
    return (
        <div className="p1-hand flex gap-4 p-2 text-primary">
            {totalPages > 1 && (<button disabled={currentPage === 0} onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 0))} className='disabled:cursor-not-allowed disabled:opacity-40'><FaCaretLeft className='text-white text-4xl bg-primary rounded-lg' /></button>)}
            ...
            {totalPages > 1 && (<button disabled={currentPage === totalPages - 1} onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages - 1))} className='disabled:cursor-not-allowed disabled:opacity-40'><FaCaretRight className='text-white text-4xl bg-primary rounded-lg' /></button>)}
